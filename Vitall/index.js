...
bot.on("message", async (msg) => { // eslint-disable-line
    ...
    if (command === "help" || command == "cmd") {
        const helpembed = new Discord.MessageEmbed()
            ...
            .setDescription(`
__**Commands List**__
> **play** > Plays a music/video with play [title/url]
> -
> **search** > Search musics/videos with search [title]
> -
> __**Other Commands**__
> \`skip\`, \`stop\`,  \`pause\`, \`resume\`
> \`nowplaying\`, \`queue\`, \`volume\``)
            .setFooter("©️ 2020 Vital Development", "");
    ...
    if (command === "play" || command === "p") {
        ...
        if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
            ...
            return msg.channel.send(`<:white_check_mark:591629527571234819>  **|**  Playlist: **\`${playlist.title}\`** has been added to the queue!`);
        } else {
            try {
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
    ...
    if (!serverQueue) {
        ...
        if (playlist) return;
        else return msg.channel.send(`<:white_check_mark:591629527571234819>  **|** **\`${song.title}\`** has been added to the queue!`);
    }
    return;
}
